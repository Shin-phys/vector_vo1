...
export const problems = {
    ...
    items: [
      {
        ...
        reveal: {
          ...
          body: '<p>基準点から地点へ向かう矢印を <b>位置ベクトル <span class="vec">r</span></b> という。</p>'
              + '<p><span class="vec">r</span> は <b>基準点</b>と<b>その先の地点</b>の、<b>どちらの情報も持っている</b>。'
              + 'どちらが欠けても意味をなさない。</p>'
              + '<p>始点の 🔒 は「置き直せない」印。</p>'
        }
      },
      {
        ...
        id: 's1q2',
        type: 'draw-vector',
        prompt: 'それでは、<b>学校</b>を基準点にした<b>駅</b>の位置ベクトルを描こう。',
        style: 'position',
        unit: 'km',
        origin: { ...MAP.school },
      ...
      },
      {
        ...
        id: 's1q3',
        type: 'draw-vector',
        prompt: 'では次に、<b>学校</b>を基準とした<b>公園</b>への位置ベクトルを描こう。',
        style: 'position',
        unit: 'km',
        origin: { ...MAP.school },
        landmarks: [{ ...MAP.park }],
        ...
        hints: ['基準点はさっきと同じ学校。公園は学校から −x に 2、+y に 3。'],
        feedback: [
    ...
    items: [
      {
        id: 's2q1',
        type: 'draw-vector',
        prompt: 'Sさんが<b>駅から公園へ</b>移動した。'
              + '<b>位置の変化を表すベクトル（変位ベクトル）</b>を描こう。'
        ...
        reveal: {
          title: 'この矢印が「変位ベクトル」',
          body: '<p>出発点から到着点へ向かう矢印を <b>変位ベクトル Δ<span class="vec">r</span></b> という。'
              + '<b>2本の位置ベクトルの、先端どうしを結んだ矢印</b>でもある。</p>'
              + '<p>Δ<span class="vec">r</span> が持っているのは <b>位置がどれだけ変化したか</b> だけ。'
              + '基準点の情報は入っていない。</p>'
      ...
      },
      {
        ...
        prompt: '★ 今度はSさんが<b>ぐねぐねした道</b>を通って、さっきと同じ<b>駅から公園へ</b>移動しました。'
              + '<br>この場合の <b>Δ<span class="vec">r</span></b> を描こう。',
        style: 'displacement',
        unit: 'km',
    ...
    items: [
      {
        id: 'sv-tap',
        type: 'tap-select',
        prompt: 'Sさんは駅から公園へ、30分かけて移動した。<br>'
              + '<b>このときの（平均の）速度と同じ向きの矢印</b>を、画面からタップして選ぼう。',
        hintText: '矢印の線そのものをタップする。',
        scene: {
          points: {
            school:  { ...MAP.school, role: 'origin' },
            station: { ...MAP.station },
  ...
  step25a: {
    ...
    items: [
      ...
      },
      {
        ...
        question: '離れた場所にいて、運動している<b>物体A</b>と<b>物体B</b>がある。'
                + 'これらの<b>変位が等しくなる</b>ことはあり得るだろうか。',
        scene: {
          points: {
      ...
      },
      {
        ...
        successText: '離れていても、2つの変位ベクトルは等しくできた。',
        hints: ['片方の成分を読み取り、もう片方を同じ成分にする。'],
        reveal: {
          title: '離れていても、2つの変位ベクトルは等しくできる',
          body: '<p>出発点も到着点もまったく別の場所なのに、2本の矢印は<b>同じ向き・同じ長さ</b>にできた。</p>'
              + '<p>変位ベクトルが持っているのは「<b>どう動いたか</b>」だけ。'
              + '「どこにいたか」は入っていないので、離れた2つの物体でも等しくなり得る。</p>'
        }
      }
    ]
  },
  ...
  step25b: {
    ...
    transition: {
      ...
      body: '<p>Δ<span class="vec">r</span> は置き直してよい矢印だった。それは、<b>Δ<span class="vec">r</span> が持っている情報が「置き場所」によらない</b>、'
          + 'ということ。</p>'
    ...
    items: [
      {
        ...
        reveal: {
          title: '先端は、動いていない',
          body: '<p>O を動かすと、O→駅 と O→公園 は<b>始点（O）だけが動き、先端（駅・公園）は動かない</b>。</p>'
              + '<p><span class="vec">r</span> は基準点の情報も持っているので、基準が変われば別の矢印になる。</p>'
              + '<p>変位は、その<b>先端どうしを結んだ矢印</b>。先端が動かないのだから、変位も動かない。</p>'
              + '<p>Δ<span class="vec">r</span> が持つのは「位置の変化」だけ。基準点とは関わりがない。</p>'
        }
      }
    ],
    // 2枚続けて出す。1枚目で「なぜ移動してよいのか」を言い切り、
    // 2枚目で 🔒 の見分け方に落とす（④の最後ではなく、基準を動かしたあとに置く）。
    summary: [
      {
        title: 'ポイント：変化を表すベクトルは、移動できる',
        body: '<p>つまり<b>変位ベクトル</b>とは、「どこにいるか」でも「どこが基準か」でもなく、'
            + '<b>「どう動いたか」という変化そのもの</b>を表す矢印。</p>'
            + '<p>だから<b>どこへ持って行ってもよい</b>。これを <b>自由ベクトル</b> として扱う。</p>'
            + '<p>Δ<span class="vec">r</span> だけの話ではない。'
            + '<b>速度ベクトルも、加速度ベクトルも移動できる。</b>どれも「変化」を表す量だから。</p>'
      ...
      },
      {
        title: '🔒 のルール',
        body: '<ul>'
            + '<li><b>位置ベクトル</b> <span class="vec">r</span> ＝ 基準点とその先の地点、'
            + '<b>どちらの情報も持つ</b>。置き直すと別ものになる（🔒 がつく＝<b>束縛ベクトル</b>）</li>'
            + '<li><b>変位ベクトル</b> Δ<span class="vec">r</span> ＝ <b>位置の変化の情報だけ</b>を持つ。'
            + 'どこに置いてもよい（<b>自由ベクトル</b>）</li>'
            + '</ul>'
            + '<p>この先も、🔒 のついた矢印は動かしても元に戻る。</p>',
        button: 'わかった'
      }
    ]
  },
    ...
    items: [
      {
        ...
        reveal: {
          title: 'なぜ運んでよいのか',
          body: '<p>第1話で確かめたとおり、<b>変位は置き直してよい矢印</b>。だから2本目を1本目の先端まで運んで継ぎ足せる。</p>'
        }
      },
      {
        id: 's3q1',
  ...
  step4: {
    title: '速度の矢印',
    minutes: 6,
    scaleLabel: '1マス = 1 km/h　ここまでとはスケールが変わる',
    transition: {
      title: 'ここから、速度の話',
    ...
    items: [
      {
        ...
        explanation: '同じ向きなので 2 + 3 ＝ 5 m/s。継ぎ足しと同じ。',
        revealVectors: [
  ...
  step6: {
    ...
    quizValue: {
      ...
      explain: '図のとおり <b>x に −5、y に +2</b>。この答えを<b>式でも出せるか</b>を確かめていく。'
    },
    coordBef: {
  ...
  /* ===================== 発展：斜方投射（第1話と第2話のあいだ） =====================
     ...
     出した問題はすべてやらせる（途中で切り上げる仕組みは置いていない）。 */
